...
# Difference from previous problem is that we need to give argument len in our
# op_modify function
...
class SegmentTree:
    def __init__(self, n):
        ...
        self.ZERO = 0  # neutral element, for bitmask it is 0
    ...
    def _update(self, l, r, v, x, lx, rx):
        ...
        if l >= rx or lx >= r:
            return
        if lx >= l and rx <= r:
            self.L[x] = self.op_modify(self.L[x], 1 << (v - 1))
            self.T[x] = self.op_modify(self.T[x], 1 << (v - 1))
            return
        mx = (lx + rx)//2
